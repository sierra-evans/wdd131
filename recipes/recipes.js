...
function tagsTemplate(tags) {
  return tags.map((tag) => `<h3 class="tags">${tag}</h3>`).join(" ");
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderFilteredRecipes(recipeList) {
        recipeContainer.innerHTML = ""; 
        ...
        recipeList.forEach(recipe => {
            ...
            recipeDiv.innerHTML = `
                <div class="recipe-image">
                    <img class="image" src="${recipe.image}" alt="Image of ${recipe.name}">
                </div>
                <div class="recipe-info">
                    <h3 class="tags">${recipe.tags.join(", ")}</h3>
